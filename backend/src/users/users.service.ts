...
export class UsersService {
  ...
  async login(loginUser: LoginUseDto) {
    ...
    const isRigthPassword = await this.comparePassword(loginUser.password, user.password);
    
    if (!isRigthPassword) {
      throw new NotFoundException("User or Password incorrect")
    }

    const generateToken = jwt.sign({id: user.id, email: user.email}, String(process.env.JWT_SECRET), {expiresIn: (process.env.JWT_EXPIRES_IN)});
